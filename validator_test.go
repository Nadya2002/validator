...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				v: struct {
					...
					Len:       []string{"abc", "bca", "cab"},
					InInt:     []int{20, 25, 30, 20},
					InStr:     []string{"foo", "bar", "foo"},
					MinInt:    []int{20, 25, 30, 20},
					MinIntNeg: []int{-1, 2, 10},
					MinStr:    []string{"abcd", "aabca", "testcab"},
					MaxInt:    []int{3, -3, 10},
					MaxIntNeg: []int{-3, -8},
					MaxStr:    []string{"abc", "abcdef", "abcdefgh"},
				},
			},
			wantErr: false,
		},
		{
			name: "wrong slice int",
			args: args{
				v: struct {
					InInt  []int `validate:"in:20,25,30"`
					MinInt []int `validate:"min:10"`
		...
		{
			name: "wrong slice string",
			args: args{
				v: struct {
					Len    []string `validate:"len:3"`
					InStr  []string `validate:"in:foo,bar"`
					MinStr []string `validate:"min:4"`
					MaxStr []string `validate:"max:2"`
				}{
					Len:    []string{"abcd", "bc", "c"},
					InStr:  []string{"abc", "bca", "cab"},
					MinStr: []string{"abc", "bc", "c"},
					MaxStr: []string{"abc", "abcdef", "abcdefgh"},
				},
			},
			wantErr: true,
